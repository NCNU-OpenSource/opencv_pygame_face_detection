...
# 載入初始介面的背景圖片
initial_screen_background = pygame.image.load('bg.JPG')  # 替換為您的初始介面背景圖片路徑
...
while run:
    ...
    if game_started:
        ...
        while game_started:
            init_progressbar(screen, x_axis, y_axis, width, height, progress)
            pygame.display.update()
            ...
            if elapsed_time<20:
                ...
                if progress > 1.0:
                    progress = 0
                update_progressbar(screen, x_axis, y_axis, width, height, progress)
